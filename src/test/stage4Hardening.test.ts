...
describe("stage 4 hardening contracts", () => {
  ...
  it("keeps the partner portal focused on operational tabs", () => {
    ...
    expect(partnerPortal).toContain('value === "pipeline" || value === "performance"');
    ...
    expect(partnerPortal).not.toContain('id: "profile"');
    expect(partnerPortal).not.toContain("const ProfileView");
