...
alter table public.partner_profile_settings add column if not exists time_off jsonb not null default '[]'::jsonb;
...
alter table public.booking_requests
  ...
  check (status in ('pending_confirmation','confirmed_unpaid','confirmed_paid','completed','no_show','cancelled','confirmed'));
...
from (
  values
    ...
    ('get_partner_workspace_rpc', to_regprocedure('public.get_partner_workspace_as_partner(text,text)') is not null),
    ('save_partner_workspace_rpc', to_regprocedure('public.save_partner_workspace_as_partner(text,text,jsonb,jsonb,jsonb,jsonb,jsonb)') is not null),
...
begin
  ...
  select exists (select 1 from public.booking_payments where booking_id = p_booking_id and status = 'paid')
...
begin
  ...
  select * into target_booking from public.booking_requests where id = p_booking_id and lawyer_id = partner_lawyer_id;
  if target_booking.id is null then
    raise exception 'BOOKING_NOT_FOUND_FOR_PARTNER' using errcode = '42501';
  end if;
  select exists (select 1 from public.booking_payments where booking_id = p_booking_id and status = 'paid')
  ...
  select * into existing_case from public.partner_cases where source_booking_id = p_booking_id limit 1;
  ...
  insert into public.partner_cases (lawyer_id, client_name, client_email, title, practice_area, source_booking_id, next_step)
  values (
    partner_lawyer_id,
  ...
  values (persisted_case.id, partner_lawyer_id, 'case_created', 'Case created from paid booking', jsonb_build_object('bookingId', target_booking.id, 'referenceId', target_booking.reference_id));
  return persisted_case;
end;
$$;
...
begin
  ...
  insert into public.partner_case_history_events(case_id, lawyer_id, event_type, message, metadata)
  values (persisted_case.id, partner_lawyer_id, 'case_updated', 'Case fields updated', jsonb_build_object('status', persisted_case.status, 'nextStep', persisted_case.next_step));
  return persisted_case;
end;
$$;
...
create or replace function public.list_partner_case_private_notes(p_partner_email text, p_session_token text, p_lawyer_id text)
...
create or replace function public.list_partner_case_history_events(p_partner_email text, p_session_token text, p_lawyer_id text)
...
set search_path = public, extensions
as $$
  select provider, provider_account_email, status, scope, connected_at, updated_at, last_error
